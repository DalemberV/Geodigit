...
% Versión: 3.0 (Con Validación de Serie de Bowen y QAPF Matemático)
% =======================================================
...
% Reglas basadas en observación visual. 
% Se usa la negación (\+) para evitar imposibles geológicos.
% -------------------------------------------------------

% --- ROCAS FÉLSICAS (Claras, ricas en sílice) ---
% REGLA DE ORO: Si tiene Cuarzo/K-Feldspato, NO debe tener Olivino.
...
identificar_visual(granito) :- 
    ...
    \+ tiene_mineral(olivino).  % Prohibido por Bowen
...
identificar_visual(diorita) :- 
    ...
    \+ tiene_mineral(cuarzo),   % Generalmente < 5% cuarzo
    \+ tiene_mineral(olivino).
...
% --- ROCAS MÁFICAS (Oscuras, ricas en Fe-Mg) ---
...
% Usa Ratios P/(A+P) para barrer el diagrama QAPF completo.
% -------------------------------------------------------

% Definimos ambientes según textura para elegir el triángulo correcto
...
% REGLA MAESTRA DE CÁLCULO
clasificar_qapf(Textura, Q, A, P, Roca) :-
    % 1. Calcular Ratio de Plagioclasa normalizado (Eje Horizontal del diagrama)
    % Fórmula: RatioP = (P / (A + P)) * 100
    ( (A + P) > 0 -> RatioP is (P / (A + P)) * 100 ; RatioP is 0 ),
    
    % 2. Derivar al diagrama correspondiente
...
% --- SUB-REGLAS PLUTÓNICAS (Triángulo Superior) ---
% Los números corresponden a los campos oficiales de la IUGS

% Campo 1a: Cuarzolita
clasificar_plutonica(Q, _, 'Cuarzolita (Silexita)') :- Q > 90.

% Campo 1b: Granitoides ricos en cuarzo
...
% Campo 6: Sienita de Feld. Alc. cuarzosa
...
% --- SUB-REGLAS VOLCÁNICAS (Simplificadas) ---
